...
- 点击架上或床上的精修 fufu，或点「看看 fufu」，可以放大独立查看。拖动旋转、滚轮 / 双指缩放；查看层支持方向键、加减键和 Home 复位，按 Esc 或「返回房间」退出。
...
新增玩偶位于床面靠枕处，左右马尾展开宽度约 40 厘米。它以高斯扫描的轮廓为约束，参考照片和视频重建五官、刘海、发饰与水手服；短绒和织物细节采用生成的法线贴图近似。书桌上方墙架上的旧版玩偶已替换为精修 fufu，床上的玩偶保留。

新增模型包含 63 个独立网格，挂在 `床 · 含床品与玩偶` 下的 `初音未来 fufu · 整体` 节点。移动或缩放该节点即可整体调整玩偶。网页加载时用共享几何和材质的副本替换墙架上的旧玩偶，按原占地宽度与朝向贴合架面；不重复下载 GLB。

独立查看层以房间原有亮度和色调为背景，显示「小鹿送我的fufu ❤️」。它使用独立相机，查看时暂停房间动画，关闭后保留房间视角与开关状态。缩放窗口或旋转手机时刷新背景。展示层销毁自身渲染资源，模型的几何和材质仍由房间统一管理。
